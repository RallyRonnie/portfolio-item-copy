...
Ext.define('CustomApp', {
	...
	createList : function(root,callback) {
		...
		async.map([config], wsapiQuery, function(err,results) {
			...
			var obj = results[0][0];
			app.list.push(obj);
			// 2 supported children - tasks, test cases
			var childRef1 = null;
			var childRef2 = null;
				...
				if (app.defined(obj.get("TestCases"))) {
					...
					if (app.defined(obj.get("Children"))){
						...
						if (app.defined(obj.get("UserStories"))) {
							childRef1 = "UserStories";
						} 
					}
				}
			}
			if (app.isObject(childRef1)) {
				var configs = [];
				var config = { reference : obj, type : childRef1 };
				configs.push(config);
				...
				async.map(configs,app.readCollection,function(err,results){
					var children = results[0].concat(results[1]?results[1]:[]);
